...
DxTextClass::DxTextClass(const DxTextClass& other)
{
}
	...
	D3DXMATRIX baseViewMatrix)
{
	...
	isSuccess = m_font->Init(device, "../LearnDx11/data/fontdata.txt", L"../LearnDx11/data/font.dds");
	if (!isSuccess)
	{
	...
	m_fontShader = new DxTextureShaderClass();
	if (!m_fontShader)
	{
		return false;
	}
	...
	isSuccess = m_fontShader->Init(device, hwnd, L"../LearnDx11/Font.vs", L"../LearnDx11/Font.ps", "FontVertexShader","FontPixelShader");
	if (!isSuccess)
	{
	...
	isSuccess = InitializeSentence(&m_sentence1, 16, device);
	if (!isSuccess)
	{
		return false;
	}
	...
	// Initialize the first sentence.
	isSuccess = InitializeSentence(&m_sentence2, 16, device);
	if (!isSuccess)
	{
		return false;
	}
...
//在屏幕上渲染句子
bool DxTextClass::Render(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX orthoMatrix)
{
	bool result;
...
bool DxTextClass::InitializeSentence(SentenceType** sentence, int maxLength, ID3D11Device* device)
{
	VertexType* vertices;
...
bool DxTextClass::UpdateSentence(SentenceType* sentence, char* text, int positionX, int positionY, float red, float green, float blue,
...
bool DxTextClass::RenderSentence(ID3D11DeviceContext* deviceContext, SentenceType* sentence, D3DXMATRIX worldMatrix,
	D3DXMATRIX orthoMatrix)
{
	...
	D3DXVECTOR4* pixelColor = new D3DXVECTOR4(sentence->red, sentence->green, sentence->blue, 1.0f);
...
//文本更新为鼠标位置信息
bool DxTextClass::SetMousePosition(int mouseX, int mouseY, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char mouseString[16];
	bool result;
